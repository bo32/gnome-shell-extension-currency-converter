...
var CurrencyConverterSettingsWidget = class CurrencyConverterSettingsWidget {
	...
	constructor() {
		...
		var display_result_in_panel_menu_switch = new Gtk.Switch({
		...
		var allow_custom_cur_switch = new Gtk.Switch({active: Settings.get_boolean('allow-custom-currencies'), halign: Gtk.Align.END});
		...
		var fav_currencies_field = new Gtk.Entry({
			hexpand: true, 
			editable: false
		});
		...
		var columns = { IS_FAVORITE: 0, CODE: 1, NAME: 2 };
		...
		var favoriteColumn = new Gtk.TreeViewColumn({
			'title': _('Favorite'), 
			'expand': false, 
			'resizable': true, 
			'clickable': true});

		var favoriteCell = new Gtk.CellRendererToggle({'mode': Gtk.CellRendererMode.ACTIVATABLE});
		...
		favoriteCell.connect('toggled', Lang.bind(this, function(cellRenderer, path) {
				var gtkpath = Gtk.TreePath.new_from_string(path);
				var selection = this._currencyTreeView.get_selection();
				selection.select_path(gtkpath);
				var [ isSelected, model, iter ] = selection.get_selected();
				...
				if (this._treeViewModel.get_value(iter, columns.IS_FAVORITE)) {
					...
					var index = fav_currencies.indexOf(this._treeViewModel.get_value(iter, columns.CODE));
					fav_currencies.splice(index, 1);
				}
				var new_fav_currencies = fav_currencies.join();
		...
		var codeColumn = new Gtk.TreeViewColumn({
			'title': _('Code'), 
			'expand': false, 
			'resizable': true});
		var codeCell = new Gtk.CellRendererText();
		...
		for(var c in Currencies.currencies) {
			var is_favorite = fav_currencies.indexOf(Currencies.currencies[c]['code']) != -1;
			var iter = this._treeViewModel.append();
			this._treeViewModel.set(iter,
		...
		var save_api_provider_button = new Gtk.Button({label: 'Save'});
		...
		/* API key field */
		var api_key_field = new Gtk.Entry({hexpand: true});
		this._grid.attach(new Gtk.Label({
			label: _('API Key'),
			halign: Gtk.Align.START,
			margin_left: MARGIN_LEFT
		...
		var test_api_key_button = new Gtk.Button({label: 'Save'});
	...
	get_text_buffer_text() {
		var start = this.text_buffer.get_start_iter();
		var end = this.text_buffer.get_end_iter();
...
function buildPrefsWidget(converter) {
    var widget = new CurrencyConverterSettingsWidget(converter);
